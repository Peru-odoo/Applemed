...
class ResConfigSettings(models.TransientModel):
	...
	def get_values(self):
		...
		partner_ids = get_param('cost_mail_alert.partner_ids', '[]')
		partner_ids = [(6, 0, literal_eval(partner_ids))]
		res.update(
			send_cost_mail_alert = self.env['ir.config_parameter'].sudo().get_param('cost_mail_alert.send_cost_mail_alert'),
	...
	def set_values(self):
		...
		param.set_param('cost_mail_alert.send_cost_mail_alert', self.send_cost_mail_alert)
		print("self.partner_ids.ids : ", self.partner_ids.ids)
		param.set_param('cost_mail_alert.partner_ids', self.partner_ids.ids)
		return res
...
class SaleOrder(models.Model):
	...
	def action_confirm(self):
		...
		if orderLines:
			send_alert = get_param('cost_mail_alert.send_cost_mail_alert')
			partner_ids = get_param('cost_mail_alert.partner_ids', '[]')
			if literal_eval(send_alert):
				...
				for partner in partners:
					...
					mail_template = self.env.ref('cost_mail_alert.cost_mail_template_alertt').id
